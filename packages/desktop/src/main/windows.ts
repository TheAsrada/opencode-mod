...
import { PINCH_ZOOM_ENABLED_KEY, WINDOW_IDS_KEY, ZOOM_FACTOR_KEY } from "./store-keys"
...
export function setPinchZoomEnabled(enabled: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (!enabled && win.webContents.getZoomFactor() !== 1) {
      setPersistedZoomFactor(1)
...
export function getPersistedZoomFactor() {
  const value = getStore().get(ZOOM_FACTOR_KEY)
  const parsed =
...
export function setPersistedZoomFactor(factor: number) {
  const clamped = clampZoom(factor)
  getStore().set(ZOOM_FACTOR_KEY, clamped)
  return clamped
}

export function applyZoomFactor(win: BrowserWindow, factor: number) {
  const clamped = setPersistedZoomFactor(factor)
  if (!win.isDestroyed()) {
...
function wireZoom(win: BrowserWindow) {
  ...
  win.webContents.setZoomFactor(getPersistedZoomFactor())
  win.webContents.on("zoom-changed", (event, zoomDirection) => {
    event.preventDefault()
    if (pinchZoomEnabled.get(win)) {
      applyZoomFactor(win, win.webContents.getZoomFactor() + (zoomDirection === "in" ? 0.2 : -0.2))
      return
    }
    applyZoomFactor(win, 1)
  })
}
